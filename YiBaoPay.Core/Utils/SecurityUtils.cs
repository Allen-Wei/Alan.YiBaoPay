...
namespace YiBaoPay.Core.Utils
{
    ...
    internal static class SecurityUtils
    {
        ...
        /// <param name="privateKey">PEM格式的base64私钥</param>
        /// <returns></returns>
        public static byte[] SignData(byte[] value, string privateKey)
        {
            var rsaProvider = DecodePemPrivateKey(privateKey);
            var signedData = rsaProvider.SignData(value, "SHA1");
            return signedData;
        }
        ...
        /// <param name="publicKey">PEM格式的base64公钥</param>
        /// <returns>是否验证成功</returns>
        public static bool VerifyData(byte[] value, byte[] sign, string publicKey)
        {
            var rsaProvider = DecodePemPublicKey(publicKey);
            return rsaProvider.VerifyData(value, "SHA1", sign);
        }
        ...
        public static byte[] AesEncrypt(string data, byte[] key)
        {
            if (key.Length != 16)
            {
                ...
                LogSystem.LogInjection.Log.Error(ex.Message, ex.StackTrace);
                throw ex;
            }
            byte[] result;
        ...
        {
            RSACryptoServiceProvider rsa = DecodePemPublicKey(key);
            if (rsa == null)
            {
                var ex = new Exception("Can't get RSACryptoServiceProvider by " + key);
        ...
        public static string RsaDecrypt(byte[] data, string key)
        {
            RSACryptoServiceProvider rsa = DecodePemPrivateKey(key);
            if (rsa == null)
            {
                var ex = new Exception("Can't get RSACryptoServiceProvider by " + key);
                LogSystem.LogInjection.Log.Error(ex.Message, ex.StackTrace);
